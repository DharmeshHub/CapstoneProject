...
***Registered Dataset:*** 
	Using the Python SDK registered dataset into Workspace from GitHub Repository (From Web files).
...
AutoML used Voting Classifier Ensemble methods, there are two types of voting method a) Soft and b) Hard voting. Hard voting is basically majority carries the vote. Here in our AutoML uses Soft voting, it predicts the class label based on argmax of the sums of the predicted probabilities of individual models that make up the ensemble. i.e weighted average the probabilities. AutoML set below parameters to avoid over-fitting.

Below are classifier and it's respective weights used in AutoML Voting Ensemble model.

- MinMaxScaler RandomForest: 0.3333333333333333
- MinMaxScaler RandomForest: 0.1111111111111111
- MinMaxScaler RandomForest: 0.1111111111111111
- MaxAbsScaler LightGBM: 0.1111111111111111
- StandardScalerWrapper LightGBM: 0.1111111111111111
- SparseNormalizer XGBoostClassifier: 0.1111111111111111
- MinMaxScaler SvcWrapper: 0.1111111111111111


Voting Ensemble estimator and weights:
![Best Runs estimator](images/FittedModel.png?raw=true "Best Run estimator") 
...
AutoML best run have higher accuracy compared to HyperDrive run. For this reason, I choose to deploy the best model from AutoML run (CapstoneAutoml.pkl, Version 2).

Registering model:

![Registered AutoML](images/AutoML_model_download.png?raw=true "Registered AutoML")

Registered model in Azure Machine Learning Studio:
...
Succesfully deployed model with Healthy service and the REST endpoint URI and Key is available:

![Deployed AutoML](images/AutoML_Endpoint_details.png?raw=true "Deployed AutoML")

Rest endpoint URL and key:

![Consume End point](images/AutoML_BestModel_Consume.png?raw=true "Consume End point")

***Consume End point:***
Created the python file endpoint.py to interact with the trained model. scoring_uri is REST endpoint point URL and Key is the authentication key generated while deploying the model using Enable Authentication.

Endpoint.py file with 3 dataset:
![Run End test file](images/endpointpy.png?raw=true "End point test file")

Make a request by executing Endpoint.py, response is in fromat of "1" or "0". 1 means predit no survival (DEATH_EVENT)
![Run End point](images/AutoML_BestModel_Consume_SDK.png?raw=true "Run End point")

Test the deployed service from Azure Machine Learning Studio.
